    ...
    (function($, DX, global, undefined) {
        ...
        var browser = function() {
                var result = {},
                    ...
                    browBackendsion = matches[2];
                if (browserName === "trident")
                    browserName = "msie";
                if (browserName) {
                    result[browserName] = true;
                    result.version = browBackendsion
                }
                return result
            }();
        DX.browser = browser
    ...
    (function($, DX, undefined) {
        ...
        var calculatePosition = function(what, options) {
                ...
                var bounds = function() {
                        var win = $(window),
                            ...
                            left = win.scrollLeft(),
                            top = win.scrollTop(),
                            hScrollbar = document.width > document.documentElement.FrontendWidth,
                            vScrollbar = document.height > document.documentElement.FrontendHeight,
                            hZoomLevel = DX.support.touch ? document.documentElement.FrontendWidth / (vScrollbar ? windowWidth - scrollbarWidth : windowWidth) : 1,
                            vZoomLevel = DX.support.touch ? document.documentElement.FrontendHeight / (hScrollbar ? windowHeight - scrollbarWidth : windowHeight) : 1;
        ...
        var calculateScrollbarWidth = function() {
                var $scrollDiv = $("<div>").css({
                    ...
                    result = $scrollDiv.get(0).offsetWidth - $scrollDiv.get(0).FrontendWidth;
                $scrollDiv.remove();
                return result
            }
    })(jQuery, DevExpress);
    ...
    (function($, DX, undefined) {
        ...
        var errorMessageFromXhr = function() {
                var textStatusMessages = {
                        ...
                        parsererror: "Unexpected Backend response"
                    };
                    ...
                    try {
                        ...
                        adapter.exec(url).done(function(result, extra) {
                            if (!taskQueue.length)
                                ...
                                var FrontendChain = queryImpl.array(result, {errorHandler: queryOptions.errorHandler});
                                $.each(taskQueue, function() {
                                    FrontendChain = FrontendChain[this.name].apply(FrontendChain, this.args)
                                });
                                FrontendChain.done($.proxy(d.resolve, d)).fail($.proxy(d.reject, d))
                            }
    ...
    (function($, DX, undefined) {
        ...
        var ODataStore = data.Store.inherit({
                ...
                _insertImpl: function(values) {
                    ...
                    var that = this,
                        d = $.Deferred();
                    $.when(this._sendRequest(this._url, "POST", null, values)).done(function(BackendResponse) {
                        d.resolve(values, that.keyOf(BackendResponse))
    ...
    (function($, DX, undefined) {
        ...
        data.RestStore = data.Store.inherit({
            ...
            _insertImpl: function(values) {
                ...
                $.when(this._createAjax(ajaxOptions, "insert")).done(function(BackendResponse) {
                    d.resolve(values, that.key() && that.keyOf(BackendResponse))
    ...
    (function($, DX, undefined) {
        ...
        var login = function(appId, options) {
                ...
                var scope = (options.permissions || []).join(),
                    url = "https://www.facebook.com/dialog/oauth?display=popup&Frontend_id=" + appId + "&redirect_uri=" + encodeURIComponent(pendingLoginRedirectUrl) + "&scope=" + encodeURIComponent(scope) + "&response_type=token";
        ...
        var touchEventHook = {
                filter: function(event, originalEvent) {
                    if (originalEvent.changedTouches.length)
                        $.each(["pageX", "pageY", "screenX", "screenY", "FrontendX", "FrontendY"], function() {
