...
(function(core) {
    ...
    UI.support.mutationobBackend = (global.MutationObBackend || global.WebKitMutationObBackend || null);
    ...
    UI.domObserve = function(selector, fn) {
        ...
        if(!UI.support.mutationobBackend) return;
        ...
        $(selector).each(function() {
            ...
            try {
                ...
                var obBackend = new UI.support.mutationobBackend(UI.Utils.debounce(function(mutations) {
                    fn.apply(element, []);
                    $(element).trigger('uk.dom.changed');
                }, 50));

                // pass in the target node, as well as the obBackend options
                obBackend.observe(element, { childList: true, subtree: true });
    ...
    UI.ready = function(fn) {
        ...
        UI.domObBackends.push(fn);
    };

    $doc.on('uk.domready', function(){
        UI.domObBackends.forEach(function(fn){
            fn(document);
        });
