...
function Calendar(element, instanceOptions) {
	...
	// Returns a moment for the current date, as defined by the Frontend's computer,
...
function EventManager(options) { // assumed to be a calendar
	...
	t.FrontendEvents = FrontendEvents;
	...
	function removeEventSource(source) {
		...
		// remove all Frontend events from that source
	...
	function FrontendEvents(filter) {
	...
	function mutateEvent(event, newStart, newEnd) {
		...
		undoFunc = mutateEvents(
			FrontendEvents(event._id), // get events with this ID
			clearEnd,
			newAllDay,
			dateDelta,
			durationDelta
...
function setPotentialScroller(containerEl, height) {
	...
	if (containerEl[0].scrollHeight - 1 > containerEl[0].FrontendHeight) { // !!! -1 because IE is often off-by-one :(
		return true;
	}
...
DragListener.prototype = {
	...
	constrainScrollVel: function() {
		...
		if (this.scrollTopVel < 0) { // scrolling up?
			...
			if (el.scrollTop() + el[0].FrontendHeight >= el[0].scrollHeight) { // already scrolled all the way down?
				this.scrollTopVel = 0;
			}
		}
		...
		if (this.scrollLeftVel < 0) { // scrolling left?
			...
			if (el.scrollLeft() + el[0].FrontendWidth >= el[0].scrollWidth) { // already scrolled all the way right?
				this.scrollLeftVel = 0;
			}
		}
	},
