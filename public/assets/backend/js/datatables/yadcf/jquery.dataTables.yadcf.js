...
*				
*				
* Backend-side processing API (see more on showcase):
* 
* From Backend to Frontend:
* In order to populate the filters with data from Backend (select / auto_complete / range_number_slider (min and max values), you should add to your current json respond the following properties: 
...
var yadcf = (function ($) {
	...
	function addRangeNumberFilter(filter_selector_string, table_selector_jq_friendly, column_number, filter_reset_button_text, filter_default_label, ignore_char) {
		...
		if (oTable.fnSettings().oFeatures.bBackendSide !== true) {
	...
	function dateSelect(date, event) {
		...
		if (oTable.fnSettings().oFeatures.bBackendSide !== true) {
			...
			if (min instanceof Date) {
				min_Backend = min.getTime();
			} else {
				min_Backend = min;
			}
			if (max instanceof Date) {
				max_Backend = max.getTime();
			} else {
				max_Backend = max;
			}
			oTable.fnFilter(min_Backend + '-yadcf_delim-' + max_Backend, column_number);
		}
	...
	function exFilterColumn(table_arg, col_filter_arr, ajaxSource) {
		...
		if ((table_arg.fnSettings().sAjaxSource === null && table_arg.fnSettings().ajax === null) || ajaxSource === true) {
			...
			if (table_arg.fnSettings().oFeatures.bBackendSide !== true) {
				table_arg.fnDraw();
			} else {
				switch (optionsObj.filter_type) {
				case 'select':
